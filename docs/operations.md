...
Расписание устанавливается на VPS при каждом deploy: ежедневно в `03:30` по
часовому поясу сервера. Проверить его можно так:

```bash
crontab -l
```

Переустановить расписание вручную без создания backup:
...
```text
локально на VPS: последние 14 дней
