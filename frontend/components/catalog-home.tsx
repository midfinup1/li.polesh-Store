...
import type { Artist, Artwork, Category, Series } from "@/types";
...
export function CatalogHome({
  ...
  const selectedSeriesId = useMemo(() => {
    if (
      activeSeriesId !== null &&
      visibleSeries.some((item) => item.id === activeSeriesId)
    ) {
      return activeSeriesId;
    }

    return null;
  }, [activeSeriesId, visibleSeries]);
    ...
    return artworks
      ...
      .filter(
        (artwork) =>
          ...
          artwork.exhibition_id === selectedSeriesId,
      )
      .filter(
        (artwork) =>
  ...
  }, [artworks, selectedCategoryId, selectedSeriesId]);
      ...
      >
        ...
        <div className="mt-12 flex gap-3 overflow-x-auto pb-1 md:gap-5">
          ...
          {visibleSeries.map((item) => {
            const isActive = selectedSeriesId === item.id;
